...
from fakevalkey import FakeServer, aioredis, FakeAsyncRedis, FakeStrictRedis
...
async def conn(async_redis: valkey.asyncio.Valkey):
    """A single connection, rather than a pool."""
    async with async_redis.client() as conn:
        yield conn


async def test_ping(async_redis: valkey.asyncio.Valkey):
    pong = await async_redis.ping()
    assert pong is True


async def test_types(async_redis: valkey.asyncio.Valkey):
...
async def test_transaction(async_redis: valkey.asyncio.Valkey):
...
async def test_transaction_fail(async_redis: valkey.asyncio.Valkey):
    ...
    async with async_redis.pipeline(transaction=True) as tr:
        ...
        tr.multi()
        tr.get("foo")
        with pytest.raises(valkey.asyncio.WatchError):
            await tr.execute()
...
@pytest.mark.slow
async def test_pubsub_timeout(async_redis: valkey.asyncio.Valkey):
...
@pytest.mark.slow
async def test_pubsub_disconnect(async_redis: valkey.asyncio.Valkey):
    async with async_redis.pubsub() as ps:
        await ps.subscribe("channel")
...
async def test_wrongtype_error(async_redis: valkey.asyncio.Valkey):
    await async_redis.set("foo", "bar")
    with pytest.raises(valkey.asyncio.ResponseError, match="^WRONGTYPE"):
        await async_redis.rpush("foo", "baz")


async def test_syntax_error(async_redis: valkey.asyncio.Valkey):
    with pytest.raises(valkey.asyncio.ResponseError, match="^wrong number of arguments for 'get' command$"):
        await async_redis.execute_command("get")


@testtools.run_test_if_lupa
class TestScripts:
    async def test_no_script_error(self, async_redis: valkey.asyncio.Valkey):
        with pytest.raises(valkey.exceptions.NoScriptError):
    ...
    async def test_failed_script_error6(self, async_redis):
        await async_redis.set("foo", "bar")
        with pytest.raises(valkey.asyncio.ResponseError, match="^Error running script"):
            await async_redis.eval('return valkey.call("ZCOUNT", KEYS[1])', 1, "foo")

    @pytest.mark.min_server("7")
    async def test_failed_script_error7(self, async_redis):
        await async_redis.set("foo", "bar")
        with pytest.raises(valkey.asyncio.ResponseError):
            await async_redis.eval('return valkey.call("ZCOUNT", KEYS[1])', 1, "foo")


@testtools.run_test_if_redispy_ver("gte", "5.1")
async def test_repr_redis_51(async_redis: valkey.asyncio.Valkey):
    assert re.fullmatch(
        r"<valkey.asyncio.connection.ConnectionPool("
        r"<fakevalkey.aioredis.FakeConnection(server=<fakevalkey._server.FakeServer object at .*>,db=0)>)>",
...
async def test_not_connected(async_redis: valkey.asyncio.Valkey):
    with pytest.raises(valkey.asyncio.ConnectionError):
...
async def test_type(async_redis: valkey.asyncio.Valkey):
...
async def test_xdel(async_redis: valkey.asyncio.Valkey):
    stream = "stream"
...
async def test_from_url():
    r0 = aioredis.FakeRedis.from_url("valkey://localhost?db=0")
    r1 = aioredis.FakeRedis.from_url("valkey://localhost?db=1")
...
async def test_from_url_with_version():
    r0 = aioredis.FakeRedis.from_url("valkey://localhost?db=0", version=(6,))
    r1 = aioredis.FakeRedis.from_url("valkey://localhost?db=1", version=(6,))
    # Check that they are indeed different databases
    await r0.set("foo", "a")
    await r1.set("foo", "b")
    assert await r0.get("foo") == b"a"
...
async def test_from_url_with_server(async_redis, fake_server):
    r2 = aioredis.FakeRedis.from_url("valkey://localhost", server=fake_server)
    await async_redis.set("foo", "bar")
...
async def test_async():
    ...
    # act
    await cache.set("fakevalkey", "plz")
    x = await cache.get("fakevalkey")
    # assert
    assert x == b"plz"
...
@pytest.mark.asyncio
async def test_cause_fakevalkey_bug(async_redis):
    if sys.version_info < (3, 11):
        return
