...
class TestInitArgs:
    def test_singleton(self):
        ...
        r1 = fakevalkey.FakeRedis()
        r2 = fakevalkey.FakeRedis(server=fakevalkey.FakeServer())
        r3 = fakevalkey.FakeRedis(server=shared_server)
        r4 = fakevalkey.FakeRedis(server=shared_server)
    ...
    def test_host_init_arg(self):
        db = fakevalkey.FakeStrictRedis(host="localhost")
        db.set("foo", "bar")
        assert db.get("foo") == b"bar"

    def test_from_url(self):
        db = fakevalkey.FakeStrictRedis.from_url("valkey://localhost:6380/0")
        db.set("foo", "bar")
        assert db.get("foo") == b"bar"

    def test_from_url_user(self):
        db = fakevalkey.FakeStrictRedis.from_url("valkey://user@localhost:6380/0")
        db.set("foo", "bar")
        assert db.get("foo") == b"bar"

    def test_from_url_user_password(self):
        db = fakevalkey.FakeStrictRedis.from_url("valkey://user:password@localhost:6380/0")
        db.set("foo", "bar")
        assert db.get("foo") == b"bar"

    def test_from_url_with_db_arg(self):
        db = fakevalkey.FakeStrictRedis.from_url("valkey://localhost:6380/0")
        db1 = fakevalkey.FakeStrictRedis.from_url("valkey://localhost:6380/1")
        db2 = fakevalkey.FakeStrictRedis.from_url("valkey://localhost:6380/", db=2)
        db.set("foo", "foo0")
        db1.set("foo", "foo1")
        db2.set("foo", "foo2")
    ...
    def test_from_url_db_value_error(self):
        ...
        db = fakevalkey.FakeStrictRedis.from_url("valkey://localhost:6380/a")
        assert db.connection_pool.connection_kwargs.get("db", 0) == 0

    def test_can_pass_through_extra_args(self):
        db = fakevalkey.FakeStrictRedis.from_url("valkey://localhost:6380/0", decode_responses=True)
        db.set("foo", "bar")
        assert db.get("foo") == "bar"

    def test_can_allow_extra_args(self):
        db = fakevalkey.FakeStrictRedis.from_url(
    ...
    def test_repr(self):
        ...
        db = fakevalkey.FakeStrictRedis.from_url("valkey://localhost:6380/11")
        rep = repr(db)
        assert "db=11" in rep

    def test_from_unix_socket(self):
        db = fakevalkey.FakeStrictRedis.from_url("unix://a/b/c")
        db.set("foo", "bar")
        assert db.get("foo") == b"bar"

    def test_same_connection_params(self):
        r1 = fakevalkey.FakeStrictRedis.from_url("valkey://localhost:6380/11")
        r2 = fakevalkey.FakeStrictRedis.from_url("valkey://localhost:6380/11")
        r3 = fakevalkey.FakeStrictRedis(server=fakevalkey.FakeServer())
        r1.set("foo", "bar")
