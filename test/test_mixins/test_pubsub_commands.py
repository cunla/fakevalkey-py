...
def test_ping_pubsub(r: valkey.Redis):
    p = r.pubsub()
    p.subscribe("channel")
...
@pytest.mark.slow
def test_pubsub_subscribe(r: valkey.Redis):
    pubsub = r.pubsub()
...
@pytest.mark.slow
def test_pubsub_numpat(r: valkey.Redis):
    p = r.pubsub()
...
@pytest.mark.slow
def test_pubsub_psubscribe(r: valkey.Redis):
    pubsub = r.pubsub()
...
@pytest.mark.slow
def test_pubsub_unsubscribe(r: valkey.Redis):
    pubsub = r.pubsub()
...
@pytest.mark.slow
def test_pubsub_punsubscribe(r: valkey.Redis):
    pubsub = r.pubsub()
...
@pytest.mark.slow
def test_pubsub_listen(r: valkey.Redis):
...
@pytest.mark.slow
def test_pubsub_listen_handler(r: valkey.Redis):
...
@pytest.mark.slow
def test_pubsub_ignore_sub_messages_listen(r: valkey.Redis):
    def _listen(pubsub, q):
        count = 0
        for message in pubsub.listen():
            q.put(message)
...
@pytest.mark.slow
def test_pubsub_binary(r: valkey.Redis):
    def _listen(pubsub, q):
        for message in pubsub.listen():
            q.put(message)
            pubsub.close()
...
@pytest.mark.slow
def test_pubsub_run_in_thread(r: valkey.Redis):
    q = Queue()
    ...
    pubsub_thread.stop()
    # Newer versions of valkey wait for an unsubscribe message, which sometimes comes early
    # https://github.com/andymccurdy/valkey-py/issues/1150
    if pubsub.channels:
        pubsub.channels = {}
    pubsub_thread.join()
    ...
    with pytest.raises(valkey.exceptions.PubSubError):
        pubsub_thread = pubsub.run_in_thread()
...
def test_pubsub_channels(r: valkey.Redis):
    p = r.pubsub()
...
def test_pubsub_channels_pattern(r: valkey.Redis):
    p = r.pubsub()
    p.subscribe("foo", "bar", "baz", "test")
...
def test_pubsub_no_subcommands(r: valkey.Redis):
...
def test_pubsub_help_redis7(r: valkey.Redis):
...
def test_pubsub_help_redis71(r: valkey.Redis):
    assert testtools.raw_command(r, "PUBSUB HELP") == [
        b"PUBSUB <subcommand> [<arg> [value] [opt] ...]. Subcommands are:",
        b"CHANNELS [<pattern>]",
        b"    Return the currently active channels matching a <pattern> (default: '*')" b".",
...
def test_pubsub_numsub(r: valkey.Redis):
    a = uuid.uuid4().hex
    b = uuid.uuid4().hex
    c = uuid.uuid4().hex
    p1 = r.pubsub()
...
def test_published_message_to_shard_channel(r: valkey.Redis):
    p = r.pubsub()
    p.ssubscribe("foo")
...
@pytest.mark.min_server("7")
@testtools.run_test_if_redispy_ver("gte", "5.0.0rc2")
def test_subscribe_property_with_shard_channels_cluster(r: valkey.Redis):
    p = r.pubsub()
...
def test_pubsub_shardnumsub(r: valkey.Redis):
...
@pytest.mark.min_server("7")
@testtools.run_test_if_redispy_ver("gte", "5.0.0rc2")
def test_pubsub_shardchannels(r: valkey.Redis):
    p = r.pubsub()
