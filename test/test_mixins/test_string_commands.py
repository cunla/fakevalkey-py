...
def test_append(r: valkey.Redis):
    assert r.set("foo", "bar")
    assert r.append("foo", "baz") == 6
    assert r.get("foo") == b"barbaz"


def test_append_with_no_preexisting_key(r: valkey.Redis):
    assert r.append("foo", "bar") == 3
    assert r.get("foo") == b"bar"


def test_append_wrong_type(r: valkey.Redis):
    r.rpush("foo", b"x")
    with pytest.raises(valkey.ResponseError):
        r.append("foo", b"x")


def test_decr(r: valkey.Redis):
    r.set("foo", 10)
    assert r.decr("foo") == 9
    assert r.get("foo") == b"9"


def test_decr_newkey(r: valkey.Redis):
    r.decr("foo")
    assert r.get("foo") == b"-1"


def test_decr_expiry(r: valkey.Redis):
    r.set("foo", 10, ex=10)
    r.decr("foo", 5)
    assert r.ttl("foo") > 0


def test_decr_badtype(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.decr("foo", 15)
    r.rpush("foo2", 1)
...
def test_get_does_not_exist(r: valkey.Redis):
    assert r.get("foo") is None


def test_get_with_non_str_keys(r: valkey.Redis):
    assert r.set("2", "bar") is True
    assert r.get(2) == b"bar"


def test_get_invalid_type(r: valkey.Redis):
    assert r.hset("foo", "key", "value") == 1
    with pytest.raises(valkey.ResponseError):
        r.get("foo")


def test_getset_exists(r: valkey.Redis):
    r.set("foo", "bar")
...
def test_getset_wrong_type(r: valkey.Redis):
    r.rpush("foo", b"x")
    with pytest.raises(valkey.ResponseError):
        r.getset("foo", "bar")


def test_getdel(r: valkey.Redis):
    r["foo"] = "bar"
    assert r.getdel("foo") == b"bar"
    assert r.get("foo") is None


def test_getdel_doesnt_exist(r: valkey.Redis):
    assert r.getdel("foo") is None


def test_incr_with_no_preexisting_key(r: valkey.Redis):
    assert r.incr("foo") == 1
    assert r.incr("bar", 2) == 2


def test_incr_by(r: valkey.Redis):
    assert r.incrby("foo") == 1
    assert r.incrby("bar", 2) == 2


def test_incr_preexisting_key(r: valkey.Redis):
    r.set("foo", 15)
    assert r.incr("foo", 5) == 20
    assert r.get("foo") == b"20"


def test_incr_expiry(r: valkey.Redis):
    r.set("foo", 15, ex=10)
    r.incr("foo", 5)
    assert r.ttl("foo") > 0


def test_incr_bad_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.incr("foo", 15)
    r.rpush("foo2", 1)
...
def test_incr_with_float(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError):
        r.incr("foo", 2.0)


def test_incr_followed_by_mget(r: valkey.Redis):
    r.set("foo", 15)
    assert r.incr("foo", 5) == 20
    assert r.get("foo") == b"20"


def test_incr_followed_by_mget_returns_strings(r: valkey.Redis):
    r.incr("foo", 1)
    assert r.mget(["foo"]) == [b"1"]


def test_incrbyfloat(r: valkey.Redis):
    r.set("foo", 0)
    assert r.incrbyfloat("foo", 1.0) == 1.0
    assert r.incrbyfloat("foo", 1.0) == 2.0


def test_incrbyfloat_with_noexist(r: valkey.Redis):
    assert r.incrbyfloat("foo", 1.0) == 1.0
    assert r.incrbyfloat("foo", 1.0) == 2.0


def test_incrbyfloat_expiry(r: valkey.Redis):
    r.set("foo", 1.5, ex=10)
    r.incrbyfloat("foo", 2.5)
    assert r.ttl("foo") > 0


def test_incrbyfloat_bad_type(r: valkey.Redis):
    r.set("foo", "bar")
    with pytest.raises(valkey.ResponseError, match="not a valid float"):
...
def test_incrbyfloat_precision(r: valkey.Redis):
    x = 1.23456789123456789
    assert r.incrbyfloat("foo", x) == x
    assert float(r.get("foo")) == x


def test_mget(r: valkey.Redis):
    r.set("foo", "one")
    r.set("bar", "two")
...
def test_mget_with_no_keys(r: valkey.Redis):
    assert r.mget([]) == []


def test_mget_mixed_types(r: valkey.Redis):
    r.hset("hash", "bar", "baz")
    r.zadd("zset", {"bar": 1})
    r.sadd("set", "member")
    r.rpush("list", "item1")
...
def test_mset_with_no_keys(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError):
        r.mset({})


def test_mset(r: valkey.Redis):
    assert r.mset({"foo": "one", "bar": "two"}) is True
    assert r.mset({"foo": "one", "bar": "two"}) is True
    assert r.mget("foo", "bar") == [b"one", b"two"]


def test_msetnx(r: valkey.Redis):
    assert r.msetnx({"foo": "one", "bar": "two"})
    assert not r.msetnx({"bar": "two", "baz": "three"})
    assert r.mget("foo", "bar", "baz") == [b"one", b"two", None]


def test_setex(r: valkey.Redis):
    assert r.setex("foo", 100, "bar") is True
    assert r.get("foo") == b"bar"


def test_setex_using_timedelta(r: valkey.Redis):
    assert r.setex("foo", timedelta(seconds=100), "bar") is True
    assert r.get("foo") == b"bar"


def test_setex_using_float(r: valkey.Redis):
    with pytest.raises(valkey.ResponseError, match="integer"):
        r.setex("foo", 1.2, "bar")


@pytest.mark.min_server("6.2")
def test_setex_overflow(r: valkey.Redis):
    with pytest.raises(ResponseError):
        r.setex("foo", 18446744073709561, "bar")  # Overflows longlong in ms


def test_set_ex(r: valkey.Redis):
    assert r.set("foo", "bar", ex=100) is True
    assert r.get("foo") == b"bar"


@pytest.mark.min_server("6.2")
def test_set_exat(r: valkey.Redis):
...
@pytest.mark.min_server("6.2")
def test_set_pxat(r: valkey.Redis):
...
def test_set_ex_using_timedelta(r: valkey.Redis):
    assert r.set("foo", "bar", ex=timedelta(seconds=100)) is True
    assert r.get("foo") == b"bar"


def test_set_ex_overflow(r: valkey.Redis):
    with pytest.raises(ResponseError):
...
def test_set_px_using_timedelta(r: valkey.Redis):
    assert r.set("foo", "bar", px=timedelta(milliseconds=100)) is True
    assert r.get("foo") == b"bar"


def test_set_conflicting_expire_options(r: valkey.Redis):
    with pytest.raises(ResponseError):
        r.set("foo", "bar", ex=1, px=1)


def test_set_raises_wrong_ex(r: valkey.Redis):
    with pytest.raises(ResponseError):
...
def test_set_using_timedelta_raises_wrong_ex(r: valkey.Redis):
    with pytest.raises(ResponseError):
...
def test_set_raises_wrong_px(r: valkey.Redis):
    with pytest.raises(ResponseError):
...
def test_set_using_timedelta_raises_wrong_px(r: valkey.Redis):
    with pytest.raises(ResponseError):
...
def test_setex_raises_wrong_ex(r: valkey.Redis):
    with pytest.raises(ResponseError):
        r.setex("foo", -100, "bar")
    with pytest.raises(ResponseError):
        r.setex("foo", 0, "bar")
...
def test_setex_using_timedelta_raises_wrong_ex(r: valkey.Redis):
    with pytest.raises(ResponseError):
...
def test_set_nx(r: valkey.Redis):
...
def test_set_xx(r: valkey.Redis):
...
@pytest.mark.min_server("6.2")
def test_set_get(r: valkey.Redis):
...
@pytest.mark.min_server("6.2")
def test_set_get_xx(r: valkey.Redis):
...
def test_set_get_nx_redis7(r: valkey.Redis):
    # Note: this will most likely fail on a 7.0 server, based on the docs for SET
    assert raw_command(r, "set", "foo", "bar", "NX", "GET") is None


@pytest.mark.min_server("6.2")
def set_get_wrongtype(r: valkey.Redis):
    r.lpush("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        raw_command(r, "set", "foo", "bar", "GET")


def test_substr(r: valkey.Redis):
    r["foo"] = "one_two_three"
...
def test_substr_noexist_key(r: valkey.Redis):
    assert r.substr("foo", 0) == b""
    assert r.substr("foo", 10) == b""
    assert r.substr("foo", -5, -1) == b""


def test_substr_wrong_type(r: valkey.Redis):
    r.rpush("foo", b"x")
    with pytest.raises(valkey.ResponseError):
        r.substr("foo", 0)


def test_strlen(r: valkey.Redis):
    r["foo"] = "bar"
...
def test_strlen_wrong_type(r: valkey.Redis):
    r.rpush("foo", b"x")
    with pytest.raises(valkey.ResponseError):
        r.strlen("foo")


def test_setrange(r: valkey.Redis):
    r.set("foo", "test")
...
def test_setrange_expiry(r: valkey.Redis):
    r.set("foo", "test", ex=10)
    r.setrange("foo", 1, "aste")
    assert r.ttl("foo") > 0


def test_large_command(r: valkey.Redis):
    r.set("foo", "bar" * 10000)
    assert r.get("foo") == b"bar" * 10000


def test_saving_non_ascii_chars_as_value(r: valkey.Redis):
    assert r.set("foo", "Ñandu") is True
    assert r.get("foo") == "Ñandu".encode()


def test_saving_unicode_type_as_value(r: valkey.Redis):
    assert r.set("foo", "Ñandu") is True
    assert r.get("foo") == "Ñandu".encode()


def test_saving_non_ascii_chars_as_key(r: valkey.Redis):
    assert r.set("Ñandu", "foo") is True
    assert r.get("Ñandu") == b"foo"


def test_saving_unicode_type_as_key(r: valkey.Redis):
    assert r.set("Ñandu", "foo") is True
    assert r.get("Ñandu") == b"foo"


def test_future_newbytes(r: valkey.Redis):
    # bytes = pytest.importorskip('builtins', reason='future.types not available').bytes
    r.set(bytes(b"\xc3\x91andu"), "foo")
    assert r.get("Ñandu") == b"foo"


def test_future_newstr(r: valkey.Redis):
    # str = pytest.importorskip('builtins', reason='future.types not available').str
    r.set(str("Ñandu"), "foo")
    assert r.get("Ñandu") == b"foo"


def test_setitem_getitem(r: valkey.Redis):
    assert r.keys() == []
    r["foo"] = "bar"
    assert r["foo"] == b"bar"


def test_getitem_non_existent_key(r: valkey.Redis):
    assert r.keys() == []
    assert "noexists" not in r.keys()


@pytest.mark.slow
def test_getex(r: valkey.Redis):
    # Exceptions
    with pytest.raises(valkey.ResponseError):
