...
from fakevalkey._fakesocket import FakeSocket
from fakevalkey._helpers import FakeSelector
from . import _msgs as msgs


class FakeConnection(FakeBaseConnectionMixin, valkey.Connection):
    def connect(self) -> None:
        super().connect()
        # The selector is set in valkey.Connection.connect() after _connect() is called
        self._selector: Optional[FakeSelector] = FakeSelector(self._sock)

    def _connect(self) -> FakeSocket:
        if not self._server.connected:
            raise valkey.ConnectionError(msgs.CONNECTION_ERROR_MSG)
    ...
    def read_response(self, **kwargs: Any) -> Any:  # type: ignore
        ...
        if isinstance(response, valkey.ResponseError):
            raise response
...
class FakeRedisMixin:
    def __init__(
        ...
        **kwargs: Any,
        ...
        parameters = list(inspect.signature(valkey.Redis.__init__).parameters.values())[1:]
        # Convert args => kwargs
        ...
        if not kwds.get("connection_pool", None):
            ...
            kwds["connection_pool"] = valkey.connection.ConnectionPool(**connection_kwargs)  # type: ignore
