...
def test_multiple_successful_watch_calls(r: valkey.Redis):
    p = r.pipeline()
    p.watch("bam")
    p.multi()
    p.set("foo", "bar")
...
def test_watch_state_is_cleared_after_abort(r: valkey.Redis):
    # valkey-py's pipeline handling and connection pooling interferes with this
    ...
    with pytest.raises(valkey.exceptions.ExecAbortError):
...
def test_pipeline_transaction_shortcut(r: valkey.Redis):
    # This example taken pretty much from the valkey-py documentation.
...
def test_pipeline_transaction_value_from_callable(r: valkey.Redis):
    def callback(pipe):
...
def test_pipeline_empty(r: valkey.Redis):
    p = r.pipeline()
    assert len(p) == 0


def test_pipeline_length(r: valkey.Redis):
    p = r.pipeline()
    p.set("baz", "quux").get("baz")
    assert len(p) == 2


def test_pipeline_no_commands(r: valkey.Redis):
    # Prior to 3.4, valkey-py's execute is a nop if there are no commands
...
def test_pipeline_failed_transaction(r: valkey.Redis):
    p = r.pipeline()
    p.multi()
    p.set("foo", "bar")
    # Deliberately induce a syntax error
    p.execute_command("set")
    # It should be an ExecAbortError, but valkey-py tries to DISCARD after the
...
def test_pipeline_srem_no_change(r: valkey.Redis):
...
# The behaviour changed in valkey 6.0 (see https://github.com/valkey/valkey/issues/6594).
@pytest.mark.min_server("6.0")
def test_pipeline_move(r: valkey.Redis):
    # A regression test for a case picked up by hypothesis tests.
    r.set("foo", "bar")
    p = r.pipeline()
    p.watch("foo")
...
def test_exec_bad_arguments(r: valkey.Redis):
...
@pytest.mark.min_server("6.0.6")
def test_exec_bad_arguments_abort(r: valkey.Redis):
...
def test_pipeline(r: valkey.Redis):
...
def test_pipeline_ignore_errors(r: valkey.Redis):
    ...
    with r.pipeline() as p:
        ...
        assert len(res) == 2
        assert isinstance(res[1], valkey.exceptions.ResponseError)


def test_pipeline_non_transactional(r: valkey.Redis):
...
def test_pipeline_raises_when_watched_key_changed(r: valkey.Redis):
    r.set("foo", "bar")
    r.rpush("greet", "hello")
    p = r.pipeline()
    try:
...
def test_pipeline_succeeds_despite_unwatched_key_changed(r: valkey.Redis):
...
def test_pipeline_succeeds_when_watching_nonexistent_key(r: valkey.Redis):
    r.set("foo", "bar")
    r.rpush("greet", "hello")
    p = r.pipeline()
    try:
...
def test_watch_state_is_cleared_across_multiple_watches(r: valkey.Redis):
    r.set("foo", "one")
    r.set("bar", "baz")
    p = r.pipeline()
...
def test_get_within_pipeline(r: valkey.Redis):
    r.set("test", "foo")
    r.set("test2", "foo2")
