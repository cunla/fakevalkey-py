...
from fakevalkey._stream import XStream, StreamRangeTest
...
def add_items(r: valkey.Redis, stream: str, n: int):
    id_list = list()
    for i in range(n):
...
def test_xadd_redis__green(r: valkey.Redis):
    stream = "stream"
...
def test_xadd_redis7(r: valkey.Redis):  # Using ts-*
    stream = "stream"
    m1 = r.xadd(stream, {"some": "other"})
    ts1, seq1 = m1.decode().split("-")
...
def test_xadd_maxlen(r: valkey.Redis):
    stream = "stream"
...
def test_xadd_minid(r: valkey.Redis):
    stream = "stream"
    id_list = add_items(r, stream, 10)
    minid = id_list[6]
...
def test_xtrim_minlen_and_length_args(r: valkey.Redis):
    stream = "stream"
    add_items(r, stream, 4)
...
def test_xadd_nomkstream(r: valkey.Redis):
...
def _add_to_stream(r: valkey.Redis, stream_name: str, n: int):
    res = []
    for _ in range(n):
...
def test_xrevrange(r: valkey.Redis):
    stream = "stream"
...
def test_xrevrange_exclusive(r: valkey.Redis):
    stream = "stream"
    m1, m2, m3, m4 = _add_to_stream(r, stream, 4)
...
def test_xrange(r: valkey.Redis):
...
def test_xread_multiple_streams_blocking(r: valkey.Redis):
    stream1 = "stream1"
    stream2 = "stream2"
...
def test_xread_blocking_no_count(r: valkey.Redis):
    k = "key"
    r.xadd(k, {"value": 1234})
    streams = {k: "0"}
...
def test_xread_count(r: valkey.Redis):
    r.xadd("test", {"x": 1})
...
def test_xgroup_destroy(r: valkey.Redis):
    stream = "stream"
    group = "group"
    r.xadd(stream, {"foo": "bar"})
...
@pytest.mark.min_server("7")
def test_xgroup_setid_redis7(r: valkey.Redis):
    stream, group = "stream", "group"
    message_id = r.xadd(stream, {"foo": "bar"})
...
def test_xgroup_delconsumer(r: valkey.Redis):
...
def test_xgroup_createconsumer(r: valkey.Redis):
    stream, group, consumer = "stream", "group", "consumer"
    r.xadd(stream, {"foo": "bar"})
    r.xadd(stream, {"foo": "bar"})
    r.xgroup_create(stream, group, 0)
...
def test_xinfo_consumers(r: valkey.Redis):
...
def test_xreadgroup(r: valkey.Redis):
...
def test_xinfo_stream(r: valkey.Redis):
    stream = "stream"
    m1 = r.xadd(stream, {"foo": "bar"})
    m2 = r.xadd(stream, {"foo": "bar"})
...
def assert_consumer_info(r: valkey.Redis, stream: str, group: str, equal_keys: List) -> List:
...
@pytest.mark.min_server("7")
def test_xinfo_stream_redis7(r: valkey.Redis):
    stream = "stream"
    m1 = r.xadd(stream, {"foo": "bar"})
    m2 = r.xadd(stream, {"foo": "bar"})
    info = r.xinfo_stream(stream)
...
def test_xinfo_stream_full(r: valkey.Redis):
    stream, group = "stream", "group"
    m1 = r.xadd(stream, {"foo": "bar"})
    r.xgroup_create(stream, group, 0)
...
def test_xpending(r: valkey.Redis):
    stream, group, consumer1, consumer2 = "stream", "group", "consumer1", "consumer2"
    m1 = r.xadd(stream, {"foo": "bar"})
    m2 = r.xadd(stream, {"foo": "bar"})
    r.xgroup_create(stream, group, 0)
...
def test_xpending_range(r: valkey.Redis):
    stream, group, consumer1, consumer2 = "stream", "group", "consumer1", "consumer2"
    m1 = r.xadd(stream, {"foo": "bar"})
    m2 = r.xadd(stream, {"foo": "bar"})
    r.xgroup_create(stream, group, 0)
...
def test_xpending_range_idle(r: valkey.Redis):
    stream, group, consumer1, consumer2 = "stream", "group", "consumer1", "consumer2"
    r.xadd(stream, {"foo": "bar"})
    r.xadd(stream, {"foo": "bar"})
    r.xgroup_create(stream, group, 0)
...
def test_xpending_range_negative(r: valkey.Redis):
    stream, group = "stream", "group"
    with pytest.raises(valkey.DataError):
...
@pytest.mark.min_server("7")
def test_xclaim_trimmed_redis7(r: valkey.Redis):
...
def test_xclaim(r: valkey.Redis):
    stream, group, consumer1, consumer2 = "stream", "group", "consumer1", "consumer2"
...
def test_stream_ttl(r: valkey.Redis):
    stream = "stream"
