...
def test_getbit(r: valkey.Redis):
    r.setbit("foo", 3, 1)
...
def test_getbit_wrong_type(r: valkey.Redis):
    r.rpush("foo", b"x")
    with pytest.raises(valkey.ResponseError):
        r.getbit("foo", 1)


def test_multiple_bits_set(r: valkey.Redis):
    r.setbit("foo", 1, 1)
    r.setbit("foo", 3, 1)
    r.setbit("foo", 5, 1)
...
def test_unset_bits(r: valkey.Redis):
    r.setbit("foo", 1, 1)
    r.setbit("foo", 2, 0)
    r.setbit("foo", 3, 1)
    assert r.getbit("foo", 1) == 1
...
def test_get_set_bits(r: valkey.Redis):
    # set bit 5
...
def test_setbits_and_getkeys(r: valkey.Redis):
...
def test_setbit_wrong_type(r: valkey.Redis):
    r.rpush("foo", b"x")
    with pytest.raises(valkey.ResponseError):
        r.setbit("foo", 0, 1)


def test_setbit_expiry(r: valkey.Redis):
    r.set("foo", b"0x00", ex=10)
    r.setbit("foo", 1, 1)
    assert r.ttl("foo") > 0


def test_bitcount(r: valkey.Redis):
    r.delete("foo")
...
def test_bitcount_mode_redis6(r: valkey.Redis):
    r.set("key", "foobar")
    with pytest.raises(valkey.ResponseError):
...
def test_bitcount_mode_redis7(r: valkey.Redis):
    r.set("key", "foobar")
...
def test_bitcount_wrong_type(r: valkey.Redis):
    r.rpush("foo", b"x")
    with pytest.raises(valkey.ResponseError):
        r.bitcount("foo")


def test_bitop(r: valkey.Redis):
    r.set("key1", "foobar")
    r.set("key2", "abcdef")
...
def test_bitop_errors(r: valkey.Redis):
    r.set("key1", "foobar")
    r.set("key2", "abcdef")
...
def test_bitpos(r: valkey.Redis):
    key = "key:bitpos"
...
@pytest.mark.min_server("7")
def test_bitops_mode_redis7(r: valkey.Redis):
    key = "key:bitpos"
    r.set(key, b"\xff\xf0\x00")
...
@pytest.mark.max_server("6.2.7")
def test_bitops_mode_redis6(r: valkey.Redis):
    key = "key:bitpos"
    r.set(key, b"\xff\xf0\x00")
    with pytest.raises(valkey.ResponseError):
        assert r.bitpos(key, 0, 8, -1, "bit") == 12
...
def test_bitpos_wrong_arguments(r: valkey.Redis):
...
def test_bitfield_empty(r: valkey.Redis):
    key = "key:bitfield"
...
def test_bitfield_wrong_arguments(r: valkey.Redis):
...
def test_bitfield_get(r: valkey.Redis):
...
def test_bitfield_set(r: valkey.Redis):
...
def test_bitfield_set_sat(r: valkey.Redis):
    key = "key:bitfield_set"
    r.set(key, b"\xff\xf0\x00")
...
def test_bitfield_set_fail(r: valkey.Redis):
    key = "key:bitfield_set"
    r.set(key, b"\xff\xf0\x00")
...
def test_bitfield_incr(r: valkey.Redis):
...
def test_bitfield_incr_sat(r: valkey.Redis):
...
def test_bitfield_incr_fail(r: valkey.Redis):
...
def test_bitfield_get_wrong_arguments(r: valkey.Redis):
...
def test_bitfield_set_wrong_arguments(r: valkey.Redis):
