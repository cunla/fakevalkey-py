...
def test_zadd_empty(r: valkey.Redis):
...
def test_zadd_minus_zero_redis6(r: valkey.Redis):
...
def test_zadd_minus_zero_redis7(r: valkey.Redis):
    r.zadd("foo", {"a": -0.0})
    r.zadd("foo", {"a": 0.0})
    assert raw_command(r, "zscore", "foo", "a") == b"0"


def test_zadd_wrong_type(r: valkey.Redis):
    r.sadd("foo", "bar")
    with pytest.raises(valkey.ResponseError):
        r.zadd("foo", {"two": 2})


def test_zadd_multiple(r: valkey.Redis):
...
def test_zadd_with_xx_and_gt_and_ch(r: valkey.Redis):
    r.zadd("test", {"one": 1})
...
def test_zadd_and_zrangebyscore(r: valkey.Redis):
