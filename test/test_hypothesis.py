...
def get_redis_version() -> Tuple[int]:
    try:
        r = valkey.StrictValkey("localhost", port=6380, db=2)
        r.ping()
        ...
        if hasattr(r, "close"):
            r.close()  # Absent in older versions of valkey-py
...
# a limited range (https://github.com/antirez/valkey/issues/5737).
dbnums = st.integers(min_value=0, max_value=3) | st.integers(min_value=-1000, max_value=1000)
# The filter is to work around https://github.com/antirez/valkey/issues/5632
...
class Command:
    ...
    @staticmethod
    def encode(arg):
        encoder = valkey.connection.Encoder("utf-8", "replace", False)
    ...
    def testable(self):
        ...
        non-deterministic, not supported, or which hits valkey bugs.
        """
        N = len(self.args)
        if N == 0:
            return False
...
common_commands = (
    ...
    # Disabled for now due to valkey giving wrong answers
    # (https://github.com/antirez/valkey/issues/5632)
...
bad_commands = (
    # valkey-py splits the command on spaces, and hangs if that ends up
    # being an empty list
...
class CommonMachine(hypothesis.stateful.RuleBasedStateMachine):
    ...
    def __init__(self):
        super().__init__()
        try:
            self.real = valkey.StrictValkey("localhost", port=6380, db=2)
            self.real.ping()
        except valkey.ConnectionError:
            pytest.skip("valkey is not running")
    ...
    def _compare(self, command):
        ...
        if fake_exc is not None and real_exc is None:
            ...
            if real_result == b"QUEUED":
                # Since valkey removes the distinction between simple strings and
...
class TestServer(BaseTest):
    # TODO: real valkey raises an error if there is a save already in progress.
